-- =====================================================
-- 22-SELLER-PAYMENT-METHODS.SQL
-- Boutique payment method configuration and checkout validation
-- =====================================================

CREATE OR REPLACE FUNCTION public.is_seller_payment_method_operational(p_method_code TEXT)
RETURNS BOOLEAN
LANGUAGE sql
IMMUTABLE
AS $$
    SELECT LOWER(COALESCE(TRIM(p_method_code), '')) IN (
        'moncash_manual',
        'natcash_manual',
        'store_pickup'
    )
...
CREATE OR REPLACE FUNCTION public.can_seller_payment_method_be_used_for_fulfillment(
    p_method_code TEXT,
...
AS $$
    SELECT
        public.is_seller_payment_method_operational(p_method_code)
        AND CASE LOWER(COALESCE(TRIM(p_method_code), ''))
            WHEN 'store_pickup' THEN LOWER(COALESCE(TRIM(p_fulfillment_method), 'delivery')) = 'pickup'
            ELSE TRUE
        END
$$;
...
CREATE TABLE IF NOT EXISTS public.seller_payment_methods (
    ...
    merchant_first_name TEXT NULL,
    merchant_last_name TEXT NULL,
    merchant_agent_code TEXT NULL,
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    CONSTRAINT seller_payment_methods_pkey PRIMARY KEY (id),
    CONSTRAINT seller_payment_methods_seller_id_fkey
        FOREIGN KEY (seller_id) REFERENCES public.sellers (id) ON DELETE CASCADE,
    CONSTRAINT seller_payment_methods_unique_key UNIQUE (seller_id, method_code),
    CONSTRAINT seller_payment_methods_method_code_check
        CHECK (
            method_code = ANY (
                ARRAY[
                    'moncash_manual'::TEXT,
...
CREATE INDEX IF NOT EXISTS idx_seller_payment_methods_seller
    ON public.seller_payment_methods USING btree (seller_id) TABLESPACE pg_default;

CREATE INDEX IF NOT EXISTS idx_seller_payment_methods_method
    ON public.seller_payment_methods USING btree (method_code) TABLESPACE pg_default;

CREATE INDEX IF NOT EXISTS idx_seller_payment_methods_active
    ON public.seller_payment_methods USING btree (seller_id, is_active) TABLESPACE pg_default;
...
CREATE OR REPLACE FUNCTION public.validate_seller_payment_method_config()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
    NEW.method_code := LOWER(TRIM(COALESCE(NEW.method_code, '')));
    ...
    IF NOT (
        ...
        RAISE EXCEPTION 'Invalid payment method code';
    END IF;

    IF NEW.is_active AND NOT public.is_seller_payment_method_operational(NEW.method_code) THEN
        RAISE EXCEPTION 'Payment method % is not operational yet', NEW.method_code;
    END IF;

    IF NEW.is_active AND NEW.method_code IN ('moncash_manual', 'natcash_manual') THEN
        IF COALESCE(TRIM(NEW.merchant_first_name), '') = '' OR COALESCE(TRIM(NEW.merchant_last_name), '') = '' THEN
            RAISE EXCEPTION 'Merchant first and last name are required for %', NEW.method_code;
        END IF;

        IF COALESCE(TRIM(NEW.merchant_agent_code), '') !~ '^[0-9]{6}$' THEN
            RAISE EXCEPTION 'Merchant agent code must contain exactly 6 digits';
        END IF;
    END IF;

    IF NEW.is_active AND NEW.method_code = 'store_pickup' THEN
        SELECT
            COALESCE((s.shipping_settings->>'allowPickup')::BOOLEAN, FALSE)
                AND COALESCE(NULLIF(TRIM(s.pickup_address->>'address'), ''), '') <> ''
        INTO v_pickup_enabled
        FROM public.sellers s
        WHERE s.id = NEW.seller_id;

        IF NOT COALESCE(v_pickup_enabled, FALSE) THEN
            RAISE EXCEPTION 'Store pickup requires pickup to be enabled in shipping settings';
        END IF;
    END IF;
...
DROP TRIGGER IF EXISTS validate_seller_payment_method_config_trigger ON public.seller_payment_methods;
CREATE TRIGGER validate_seller_payment_method_config_trigger
    BEFORE INSERT OR UPDATE ON public.seller_payment_methods
    FOR EACH ROW
    EXECUTE FUNCTION public.validate_seller_payment_method_config();

ALTER TABLE public.seller_payment_methods ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Sellers can read own payment methods" ON public.seller_payment_methods;
CREATE POLICY "Sellers can read own payment methods"
...
DROP POLICY IF EXISTS "Sellers can insert own payment methods" ON public.seller_payment_methods;
CREATE POLICY "Sellers can insert own payment methods"
    ON public.seller_payment_methods
    FOR INSERT
    WITH CHECK (
        seller_id = public.current_seller_id()
...
DROP POLICY IF EXISTS "Sellers can update own payment methods" ON public.seller_payment_methods;
...
DROP POLICY IF EXISTS "Sellers can delete own payment methods" ON public.seller_payment_methods;
