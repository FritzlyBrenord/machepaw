...
const FALLBACK_CURRENCIES: Array<{ code: CurrencyCode; symbol: string }> = [
  { code: "HTG", symbol: "G" },
  { code: "USD", symbol: "$" },
  { code: "EUR", symbol: "EUR" },
  { code: "DOP", symbol: "RD$" },
...
function getErrorMessage(error: unknown) {
  ...
  return "Erreur lors de la creation du produit vendeur.";
}
...
function hasAttributeValue(value: ProductAttributeValue["value"] | undefined) {
...
export default function SellerAddProductPage() {
  ...
  const handleSubmit = async () => {
    ...
    const parsedMinProcessingDays = parseInt(formData.minProcessingDays, 10);
    const parsedMaxProcessingDays = parseInt(formData.maxProcessingDays, 10);
    ...
    if (formData.imageFiles.length === 0) {
      setError("Ajoutez au moins une image produit.");
      setCurrentStep(2);
      return;
    }

    if (missingRequiredAttributeLabels.length > 0) {
      setError(
        `Renseignez tous les attributs dynamiques obligatoires: ${missingRequiredAttributeLabels.join(", ")}.`,
      );
      setCurrentStep(3);
      return;
    }
    ...
    if (
      ...
      parsedMinProcessingDays < 1 ||
      parsedMaxProcessingDays < parsedMinProcessingDays
    ) {
      setError(
    ...
    try {
      ...
      await createProduct.mutateAsync({
        ...
        price: parsedPrice,
        originalPrice:
          parsedOriginalPrice && Number.isFinite(parsedOriginalPrice)
            ? parsedOriginalPrice
            : undefined,
        stock: parsedStock,
        sku: formData.sku.trim(),
        currencyCode: formData.currencyCode as "HTG" | "USD" | "EUR" | "DOP",
        tags: formData.tags
          .split(",")
          .map((t) => t.trim())
          .filter(Boolean),
      ...
      const errorMessage = getErrorMessage(submissionError);

      console.error(
        "Error creating product:",
        serializeErrorForConsole(submissionError),
      );
  ...
  const canProceed = () => {
    switch (currentStep) {
      ...
      case 3:
        return missingRequiredAttributeLabels.length === 0;
      case 4:
        return Boolean(
          formData.currencyCode &&
            formData.sku.trim() &&
        ...
        <div className="min-h-[60vh] flex items-center justify-center">
          ...
          >
            ...
            <p className="text-neutral-500 mb-8 max-w-md mx-auto">
              Le produit a été publié dans votre boutique vendeur et peut
              maintenant recevoir des commandes.
            </p>
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-neutral-500">
              Renseignez tous les champs obligatoires avant de publier dans
            ...
            <div className="space-y-6">
              ...
              {formData.subcategory ? (
                <div className="space-y-4">
                  ...
                  {missingRequiredAttributeLabels.length > 0 ? (
                    <div className="rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
                      Attributs obligatoires manquants:{" "}
                      {missingRequiredAttributeLabels.join(", ")}.
                    </div>
                  ) : null}
