...
export default function SellerStatisticsPage() {
  ...
  const { data: orderItems = [], isLoading: ordersLoading } =
    useSellerOrderItemsQuery();
  ...
  const dailySeries = Array.from(
    { length: Math.min(rangeInDays, 14) },
    (_, index) => {
  ...
  const maxSeriesValue = Math.max(
    ...dailySeries.map((item) => item.revenue),
    1,
  );
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <div className="rounded-2xl border border-neutral-200 bg-white p-5">
          ...
          <div className="mt-2 flex items-center gap-2 text-xs text-neutral-500">
            {
              filteredItems.filter((item) => item.itemStatus === "cancelled")
                .length
            }{" "}
            annulee(s)
          </div>
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-neutral-500">
              Serie recentre sur les ventes de votre boutique.
            </p>
          </div>
        </div>
              ...
              >
                <div className="group relative flex h-60 w-full items-end">
                  <div
                    ...
                    style={{
                      height: `${Math.max((item.revenue / maxSeriesValue) * 100, item.revenue > 0 ? 8 : 2)}%`,
                    }}
                  />
            ...
            <tbody className="divide-y divide-neutral-200">
              {productsLoading || ordersLoading ? (
                ...
                topProducts.map((product) => {
                  const catalogEntry = products.find(
