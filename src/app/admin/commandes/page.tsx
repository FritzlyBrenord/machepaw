...
export default function AdminOrdersPage() {
  ...
  const handleStatusChange = async (orderId: string, status: OrderStatus) => {
    ...
    if (status === "shipped") {
      trackingNumber =
        prompt("Entrez le numéro de suivi (optionnel):") || undefined;
    }
                                ...
                                <button
                                  onClick={async () => {
                                    try {
                                      ...
                                      alert(
                                        `Erreur DB: ${err.message || JSON.stringify(err)}`,
                                      );
                                      console.error(err);
                                    }
                                  }}
                ...
                <div className="p-6 space-y-6">
                      ...
                      <div className="space-y-1 text-sm">
                        ...
                        <div className="flex justify-between">
                          <span className="text-neutral-500">
                            {selectedOrder.notes?.includes("[RETRAIT")
                              ? "Frais de retrait :"
                              : "Livraison :"}
                          </span>
                          <span className="font-medium">
                                    ...
                                    {selectedOrder.estimatedDelivery && (
                                      <button
                                        onClick={async () => {
                                          if (
                                            confirm(
                                              "Supprimer la date limite personnalisée ?",
                                            )
                                          ) {
                                            await updateDeliveryDateMutation.mutateAsync(
                                              {
                      ...
                      <div className="bg-neutral-50 p-4 rounded-lg space-y-3">
                        ...
                        {selectedOrder.paymentId && (
                          <div className="flex justify-between items-center">
                            ...
                            <span className="font-mono bg-neutral-200 px-2 rounded">
                              {selectedOrder.paymentId}
                            </span>
                          </div>
                        )}
                        {selectedOrder.paymentProofUrl && (
                          <div>
                            <p className="text-sm text-neutral-600 mb-2">
                              Preuve de paiement (cliquez pour agrandir):
                            </p>
                            <button
                              onClick={() =>
                                setEnlargedImage(selectedOrder.paymentProofUrl!)
                              }
                  ...
                  <div className="flex gap-3">
                    ...
                    {!(
                      ...
                      selectedOrder.paymentMethod === "delivery" ||
                      selectedOrder.paymentMethod === "store_pickup"
                    ) &&
