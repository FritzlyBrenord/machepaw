...
export async function GET(
  ...
  return NextResponse.json({
    addresses: (data || []).map((row) => mapBoutiqueAddressRow(row as never)),
  });
}
...
export async function POST(
  ...
  const payload = {
    boutique_customer_id: session.customer.id,
    seller_id: session.store.id,
  ...
  if (!payload.first_name || !payload.last_name || !payload.address || !payload.phone) {
    return NextResponse.json(
      { error: "Informations d'adresse insuffisantes." },
      { status: 400 },
    );
  }

  if (payload.is_default) {
    await clearDefaultAddresses(session.customer.id, session.store.id);
  }
  ...
  if (error || !data) {
    return NextResponse.json(
      { error: error?.message || "Creation de l'adresse impossible." },
      { status: 500 },
    );
  }
